...
// Mock clipboard API - create a persistent mock that can be tracked
...
describe("TeamSettingsPage", () => {
  ...
  describe("Invitations Tab", () => {
    ...
    it("should display resend button next to each pending invitation", async () => {
      render(<TeamSettingsPage />);
    ...
    it("should call resendInvitation when resend button is clicked", async () => {
      (resendInvitation as ReturnType<typeof vi.fn>).mockResolvedValue({
        success: true,
        emailSent: true,
      });
      ...
      await waitFor(() => {
        expect(resendInvitation).toHaveBeenCalledWith("team-1", "invite-1");
      });
    });

    it("should show loading state while resending invitation", async () => {
      let resolveResend: (value: unknown) => void;
      const resendPromise = new Promise((resolve) => {
        resolveResend = resolve;
      });
      (resendInvitation as ReturnType<typeof vi.fn>).mockReturnValue(resendPromise);
      ...
      await waitFor(() => {
        expect(screen.getByRole("button", { name: /resending/i })).toBeInTheDocument();
      });

      // Resolve the promise
      resolveResend!({ success: true, emailSent: true });
    ...
    it("should show success message when email is resent successfully", async () => {
      (resendInvitation as ReturnType<typeof vi.fn>).mockResolvedValue({
        success: true,
        emailSent: true,
      });
    ...
    it("should show invite link with copy button when email fails", async () => {
      (resendInvitation as ReturnType<typeof vi.fn>).mockResolvedValue({
        success: true,
        emailSent: false,
    ...
    it("should copy invite link to clipboard when copy button is clicked", async () => {
      // Reset the clipboard mock for this specific test
      ...
      // Wait for "Copied!" text to confirm clipboard operation completed
      await waitFor(() => {
        expect(screen.getByText(/copied/i)).toBeInTheDocument();
      });

      // Verify the clipboard was called with correct link
      expect(mockWriteText).toHaveBeenCalledWith("https://example.com/invite/abc123");
    });

    it("should show copied confirmation after copying link", async () => {
      (resendInvitation as ReturnType<typeof vi.fn>).mockResolvedValue({
        success: true,
        emailSent: false,
        emailError: "SMTP server unavailable",
    ...
    it("should handle resend error gracefully", async () => {
      (resendInvitation as ReturnType<typeof vi.fn>).mockRejectedValue(
    ...
    it("should show email status feedback after sending new invitation", async () => {
      (sendInvitation as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "invite-2",
        email: "newuser@example.com",
        role: "viewer",
    ...
    it("should show invite link when email fails on new invitation", async () => {
      (sendInvitation as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "invite-2",
        email: "newuser@example.com",
        role: "viewer",
  ...
  describe("Access Control", () => {
    ...
    it("should show billing tab only for owners", async () => {
      // Owner should see billing tab
      render(<TeamSettingsPage />);
    ...
    it("should not show billing tab for admins", async () => {
      (getTeam as ReturnType<typeof vi.fn>).mockResolvedValue({
        ...mockTeam,
        role: "admin",
      });
    ...
    it("should not show billing tab for editors", async () => {
      (getTeam as ReturnType<typeof vi.fn>).mockResolvedValue({
        ...mockTeam,
        role: "editor",
      });
    ...
    it("should show advanced tab for owners", async () => {
      render(<TeamSettingsPage />);
    ...
    it("should show advanced tab for admins", async () => {
      (getTeam as ReturnType<typeof vi.fn>).mockResolvedValue({
        ...mockTeam,
        role: "admin",
      });
    ...
    it("should not show advanced tab for editors", async () => {
      (getTeam as ReturnType<typeof vi.fn>).mockResolvedValue({
        ...mockTeam,
        role: "editor",
      });
    ...
    it("should not show advanced tab for viewers", async () => {
      (getTeam as ReturnType<typeof vi.fn>).mockResolvedValue({
        ...mockTeam,
        role: "viewer",
      });
