...
import { createTeamApi, apiUpload, ApiError } from "@/lib/api-client";
...
export function CreateDataSourceDrawer({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (sourceType === "csv-upload" && file) {
        ...
        const data = await apiUpload<{ id: string }>(
  ...
  const handleApiPushCreate = useCallback(async () => {
    ...
    try {
      const response = await teamApi.post<{ id: string }>("/api/v1/data-sources", {
        name: name.trim(),
        type: "api",
  ...
  const handleApiKeyGenerated = useCallback(async () => {
    ...
    try {
      const data = await teamApi.get<{ config?: { apiKey?: ApiKeyConfig } }>(`/api/v1/data-sources/${apiDataSourceId}`);
  ...
  const handleCsvPasteSubmit = useCallback(
    async (dataSourceName: string, csvContent: string) => {
      ...
      try {
        ...
        const previewData = await teamApi.post<{ preview: Record<string, unknown>[] }>(
        ...
        const createResponse = await teamApi.post<{ id: string }>("/api/v1/data-sources", {
          name: dataSourceName,
          type: "csv",
          config: { source: "paste" },
        });
        ...
        await teamApi.post(`/api/v1/data-sources/${dataSourceId}/items`, {
          mode: "replace",
          items: parsedItems,
        });
   ...
   * Requires userId to look up the user's OAuth tokens
   */
  const checkGoogleConnection = useCallback(async () => {
    ...
    try {
      const response = await teamApi.get<{ connected: boolean }>(
   ...
   * Requires userId to associate the OAuth tokens with the user
   */
  const handleGoogleConnect = useCallback(async () => {
    ...
    try {
      const response = await teamApi.post<{ authorizationUrl: string }>(
        "/api/v1/auth/google/connect",
        {
          // Pass current URL for redirect after OAuth
          redirectUrl: window.location.href,
          // Include userId in body for the OAuth flow
          userId: user.id,
        },
        { headers: { "X-User-Id": user.id } }
      );
   ...
   * Passes userId so the backend knows whose credentials to use for syncing
   */
  const handleGoogleSheetsSubmit = useCallback(
    async (dataSourceName: string, config: GoogleSheetsConfig) => {
      if (!user?.id) {
        setError("You must be logged in to create a Google Sheets data source");
        return;
      }
      ...
      try {
        const response = await teamApi.post<{ id: string }>(
          "/api/v1/data-sources",
          {
            name: dataSourceName,
            type: "google-sheets",
          ...
          // Pass userId header so the sync job knows whose credentials to use
          { headers: { "X-User-Id": user.id } }
        );
