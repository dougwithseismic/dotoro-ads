...
export function GenerationPreview(props: GenerationPreviewProps) {
  ...
  const generateButtonText = useMemo(() => {
    ...
    const campaignText = `${stats.perPlatformCampaigns} Campaign${stats.perPlatformCampaigns !== 1 ? "s" : ""}`;
    if (selectedPlatforms.length > 1) {
      return `Generate ${campaignText} for ${selectedPlatforms.length} Platforms`;
    }
    return `Generate ${stats.perPlatformCampaigns} Campaign${stats.perPlatformCampaigns !== 1 ? "s" : ""}`;
  }, [generating, stats.perPlatformCampaigns, selectedPlatforms.length]);
