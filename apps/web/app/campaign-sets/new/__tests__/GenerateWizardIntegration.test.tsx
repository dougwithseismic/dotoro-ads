...
import CreateCampaignSetPage from "../page";
 ...
 * 8. preview       - Review and create campaign set
 *
 * REQUIRED UPDATES:
 ...
 *
 * @see /apps/web/app/campaign-sets/new/components/CampaignEditor.tsx
 */
describe.skip("Campaign Generation Wizard Integration", () => {
  ...
  it("renders with initial step data-source", async () => {
    render(<CreateCampaignSetPage />);

    // Should show page heading
    expect(screen.getByRole("heading", { name: "Campaign Set Builder" })).toBeInTheDocument();
  ...
  it("shows all 6 steps in the step indicator", async () => {
    ...
    expect(within(stepIndicator).getByText("Preview & Create")).toBeInTheDocument();
  });
  ...
  it("displays page header correctly", () => {
    ...
    expect(screen.getByRole("heading", { name: "Campaign Set Builder" })).toBeInTheDocument();
    expect(
      screen.getByText(/create a campaign set from your data/i)
    ).toBeInTheDocument();
  });
