...
describe('validateWizardStep', () => {
  ...
  describe('preview step', () => {
    it('returns error if campaign set name is not provided', () => {
      const state = createInitialState();
      // campaignSetName is empty by default
      const result = validateWizardStep('preview', state);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Campaign set name is required');
    });

    it('returns error if data source not selected', () => {
      const state = createInitialState();
      state.campaignSetName = 'My Campaign Set'; // Set valid campaign set name
      const result = validateWizardStep('preview', state);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Please select a data source');
    });
  ...
  describe('campaign-set-name step', () => {
    it('returns error when campaign set name is empty', () => {
      const state = createInitialWizardState();
      state.campaignSetName = '';
      const result = validateWizardStep('campaign-set-name', state);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Campaign set name is required');
    });

    it('returns error when campaign set name is whitespace only', () => {
      const state = createInitialWizardState();
      state.campaignSetName = '   ';
      const result = validateWizardStep('campaign-set-name', state);
      expect(result.valid).toBe(false);
    ...
    it('returns error when campaign set name is too short', () => {
      const state = createInitialWizardState();
      state.campaignSetName = 'Ab';
      const result = validateWizardStep('campaign-set-name', state);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Campaign set name must be at least 3 characters');
    });

    it('returns error when campaign set name is too long', () => {
      const state = createInitialWizardState();
      state.campaignSetName = 'a'.repeat(256);
      const result = validateWizardStep('campaign-set-name', state);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Campaign set name must be at most 255 characters');
    });

    it('returns valid when campaign set name is exactly 3 characters', () => {
      const state = createInitialWizardState();
      state.campaignSetName = 'Abc';
      const result = validateWizardStep('campaign-set-name', state);
      expect(result.valid).toBe(true);
      expect(result.errors).toHaveLength(0);
    });

    it('returns valid when campaign set name is exactly 255 characters', () => {
      const state = createInitialWizardState();
      state.campaignSetName = 'a'.repeat(255);
      const result = validateWizardStep('campaign-set-name', state);
      expect(result.valid).toBe(true);
      expect(result.errors).toHaveLength(0);
    });

    it('returns valid when campaign set name is normal length', () => {
      const state = createInitialWizardState();
      state.campaignSetName = 'My Campaign Set';
      const result = validateWizardStep('campaign-set-name', state);
      expect(result.valid).toBe(true);
      expect(result.errors).toHaveLength(0);
    });
...
describe('validateCampaignSetName', () => {
  it('returns error when name is empty', () => {
    const state = createInitialWizardState();
    state.campaignSetName = '';
    const result = validateCampaignSetName(state);
    expect(result.valid).toBe(false);
    expect(result.errors).toContain('Campaign set name is required');
  });
  ...
  it('returns error when name is more than 255 characters', () => {
    const state = createInitialWizardState();
    state.campaignSetName = 'a'.repeat(256);
    const result = validateCampaignSetName(state);
    expect(result.valid).toBe(false);
  ...
  it('returns valid when name is exactly 3 characters', () => {
    const state = createInitialWizardState();
    state.campaignSetName = 'Abc';
    const result = validateCampaignSetName(state);
    expect(result.valid).toBe(true);
  ...
  it('returns valid when name is exactly 255 characters', () => {
    const state = createInitialWizardState();
    state.campaignSetName = 'a'.repeat(255);
    const result = validateCampaignSetName(state);
    expect(result.valid).toBe(true);
  ...
  it('returns valid when name is normal length', () => {
    const state = createInitialWizardState();
    state.campaignSetName = 'Q4 Brand Campaign Set';
    const result = validateCampaignSetName(state);
    expect(result.valid).toBe(true);
    expect(result.errors).toHaveLength(0);
  });
  ...
  it('does not validate description (optional)', () => {
    const state = createInitialWizardState();
    state.campaignSetName = 'Valid Name';
    state.campaignSetDescription = ''; // Empty description should be fine
    const result = validateCampaignSetName(state);
    expect(result.valid).toBe(true);
  });

  it('accepts description with any length', () => {
    const state = createInitialWizardState();
    state.campaignSetName = 'Valid Name';
    state.campaignSetDescription = 'a'.repeat(1000);
    const result = validateCampaignSetName(state);
    expect(result.valid).toBe(true);
  });
});
...
describe('createInitialWizardState', () => {
  it('creates state with empty campaignSetName', () => {
    const state = createInitialWizardState();
    expect(state.campaignSetName).toBe('');
  });

  it('creates state with empty campaignSetDescription', () => {
    const state = createInitialWizardState();
    expect(state.campaignSetDescription).toBe('');
  });

  it('creates state with currentStep as campaign-set-name', () => {
    const state = createInitialWizardState();
    expect(state.currentStep).toBe('campaign-set-name');
  });

  it('creates state with all other required fields', () => {
    const state = createInitialWizardState();
    expect(state.dataSourceId).toBeNull();
    expect(state.availableColumns).toEqual([]);
    expect(state.campaignConfig).toBeNull();
    expect(state.hierarchyConfig).toBeNull();
